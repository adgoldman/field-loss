...
function modelState(crop, area, wx, priceOverride, importFactor = 1) {
  const c = CROPS[crop];
...
export default function FieldLossMap({ onDrill, importShock = 0, setImportShock }) {
  ...
  // at shock = live YoY change the factor is ×1.00 (reproduces today's real market)
  // and only the swing away from it moves price. yoyAnchor=0 → status-quo anchor.
  const importYoy = imp?.live?.yoyPct ?? 0;
  const pfMap = (x) => clamp(1 - (imp?.priceFlex ?? 2.0) * x * (imp?.importShare ?? 0), 0.25, 2.5);
  const importFactor = clamp(pfMap(importShock) / pfMap(importYoy), 0.25, 2.5);

  // auto-set the shared lever to the live YoY import change on FAS load (once per
  // crop), so the map opens on the real current market. Guard on imp.crop because
  // the fetch is async and imp can briefly hold the previous crop's payload.
  ...
  const data = useMemo(()=>{
    ...
    Object.keys(STATES).forEach(s=>{
      ...
      const m=modelState(crop,area,wx.byState[s],livePrice?.price,importFactor);
      out[s]=m;
  ...
  }, [crop, metric, wx, acresInfo, livePrice, importFactor]);
...
// Shared import-shock lever (same model + state used by every tab). Moving it here
// flows through importFactor → state price → economic abandonment → loss totals.
function ImportShockBar({ crop, imp, importShock, setImportShock, importFactor }){
